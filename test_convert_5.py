        ...
        for row in ws.iter_rows():
            ...
            for cell in row:
                ...
                if cell.value is not None:
                    ...
                    if is_merged:
                        # Lấy địa chỉ ô hợp nhất
                        for merged_cell in ws.merged_cells.ranges:
                            if cell.coordinate in merged_cell:
                                # Lấy giá trị từ ô đầu tiên trong vùng hợp nhất
                                value = ws[merged_cell.start_cell.coordinate].value
                                # Lấy địa chỉ góc trên bên trái
                                top_left = merged_cell.start_cell.coordinate
                                # Tính toán địa chỉ góc dưới bên phải
                                bottom_right_row = merged_cell.bounds[3]  # Row number of the bottom-right corner
                                bottom_right_col = merged_cell.bounds[2]  # Column number of the bottom-right corner
